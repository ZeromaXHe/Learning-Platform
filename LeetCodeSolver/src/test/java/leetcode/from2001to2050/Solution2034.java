...
/**
 * @author zhuxi
 * @apiNote 2034. 股票价格波动 | 难度：中等 | 标签：设计、哈希表、数据流、有序集合、堆（优先队列）
...
public class Solution2034 {
     ...
     * <p>
     * 时间 113 ms
     * 击败 55.79% 使用 Java 的用户
     * 内存 100.12 MB
     * 击败 48.42% 使用 Java 的用户
     */
    class StockPrice {
        ...
        public void update(int timestamp, int price) {
            ...
            if (timePriceMap.containsKey(timestamp)) {
                ...
                if (prevPrice > 0) {
                    int count = priceCountMap.get(prevPrice);
                    if (count == 1) {
                        priceCountMap.remove(prevPrice);
                    } else {
                        priceCountMap.put(prevPrice, count - 1);
                    }
                }
                priceCountMap.put(timestamp, price);
            }
